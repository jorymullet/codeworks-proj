...
exports.getByEmail = async (req, res) => {
    ...
    if (!email)
        res.status(404).send({ error: true, message: 'Could not create full nav since we could not find your uid based on your email.' });
    let userAuth;
    try {
    ...
    if (!(user && user.role))
        return res.status(404).send({ error: true, message: 'Could not create full nav since we could not find a user role based on this email.' });
