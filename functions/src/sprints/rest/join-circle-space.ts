...
export const joinCircleSpace = async (req: Request, res: Response) => {
  // -- Validate
  const {sprintId, userId} = req.params

  if (!(sprintId && userId)) return res.status(400).send({message: 'Must include sprint ID and user ID.'})
  ...
  if (!(sprint && user)) return res.status(400).send({message: 'Sprint and/or user do not exist.'})
  ...
  if (response.success === false) {
    ...
    if (response.message === 'User was not added to space') return res.status(200).send() // it's cool that the user was already added
