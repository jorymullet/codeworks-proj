...
export const inviteUsersToSprint = async (req: Request, res: Response) => {

  const {users, origin} = req.body
  const sprintId = req.params.sprintId

  if (!(users && users.length && origin && sprintId)) return res.status(400).send({message: 'Invalid request. Must includes userIds and origin.'})

  const user = req.user.data

  const isAdmin = ['SUPER_ADMIN', 'ADMIN'].includes(user.role)
  if (!isAdmin) return res.status(403).send({message: 'Must be an admin.'})
  ...
  if (userNotFromOrg) return res.status(403).send({message: 'You cannot send invites to users of different orgs'})
  ...
  const invitesPromises = users.map(async u => {
    const invite = {
      id: `${u.id}_${sprintId}`, // constructing the id like this prevents duplicate invites
      created_at: now,
      created_by: user.id,
      updated_at: now,
      updated_by: user.id,
