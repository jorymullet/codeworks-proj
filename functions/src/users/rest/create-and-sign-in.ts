...
export const post = async (req: Request, res: Response) => {
  ...
  if (missingFields.length) return res.status(400).send({message: `Request must include ${missingFields}`})
  ...
  try {
    ...
    if (tokenRes.error) return res.status(500).send({message: 'Could not retrieve token.', retry: true})
  ...
  if (!user) {
    ...
    if (!pendingUser) return res.status(400).send({message: `There is no user associated with this ID ${uid} and no pending user was sent to create another.`})

    const inviteId = pendingUser.invite_id
    if (!inviteId) return res.status(400).send({message: `There is invite_id associated with this pending user so a user cannot be created.`})
    ...
    if (!isValidInvite) return res.status(400).send({message: `There is either no invite associated with this user OR the invite is not PENDING.`})

    // -- Update Auth0 Picture
    try {
      await updateAuth0Picture(uid, pendingUser.photo_url)
    } catch (err) {
      console.error(err)
    ...
    try {
      ...
      console.error(err)
      return res.status(400).send({message: err?.errorInfo?.message || `Could not create Firebase Auth.`, error: err,})
    }
...
export const updateAuth0Picture = (id, picture) => new Promise(async (resolve, reject) => {
  try {
