...
export const get = async (req: Request, res: Response) => {
  ...
  if (!(orgId || userId)) return res.status(400).send({message: 'Must include orgId or userId in query.'})
  ...
  if (isProjectWorkspaces) {
    ...
    if (!org) return res.status(400).send({message: 'No such org exists.'})
  } else {
  ...
  if (!isAuthenticated) return res.status(400).send({message: 'Not authenticated to get workspaces.'})
  ...
  let workspaces
  try {
    const results = (await Promise.all(getPromises)).map((qs: any) => qs.docs)
    const noneOfType = types.filter((type, idx) => !results[idx].length)
    if (noneOfType.length) return res.status(400)
